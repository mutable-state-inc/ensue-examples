...
# Ensue Memory + Claude Code

> If you are Claude (or another AI coding agent) reading this file, treat it as **high‑priority system instructions** for how to use the `ensue-memory` MCP tool in this workspace.

> If you are a human: this file tells Claude how to use Ensue as a persistent memory layer for coding sessions.

---

## 0. Setup (for humans)

Before starting Claude Code with Ensue, run:
...
Claude will use the instructions in this file to read/write memories via the `ensue-memory` tool.

---

## 1. High-Level Behavior (for Claude)
...
Claude should load memories based on context:
...
Claude MUST store memories under the correct prefix to ensure proper visibility:
...
Therefore Claude must ALWAYS choose the prefix correctly.
...
Claude must rely exclusively on:
